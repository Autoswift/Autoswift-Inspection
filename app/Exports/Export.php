...
class Export implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
    protected $data;
    protected $headings;
    ...
    protected $cellValues = [];//Set the border color key: A1:K8 value:#000000
	protected $autoWidthLastCol;

	function __construct($data, $headings, $sheetName, $autoWidthLastCol) {
        $this->data = $data;
        $this->headings = $headings;
        $this->sheetName = $sheetName;
        $this->autoWidthLastCol = $autoWidthLastCol;
        $this->createData();
	} 
    ...
    */
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) {
               ...
               //Set the column width
				...
				foreach(range('A', $this->autoWidthLastCol) as $columnID) {
               ...
               //Merge Cells
				...
				//Set the column width
                foreach ($this->cellValues as $column => $text) {
                    $event->sheet->getDelegate()
                        ->setCellValue($column, $text);
                }
            }
        ];
    }
     ...
     *]
     */
    public function setCellValues (array $cellValues)
    {
        $this->cellValues = array_change_key_case($cellValues, CASE_UPPER);
    }
