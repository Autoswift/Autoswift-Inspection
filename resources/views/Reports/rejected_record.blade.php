...
<div id="page-wrapper">
   <div class="container-fluid">
               ...
               <table class="table table-bordered table-hover table-striped" id="report_table" style="width:100%;table-layout: fixed;background-color: transparent;border-spacing: 0;border-collapse: collapse;">
                  <thead>
                     <tr>
                        ...
                        <th style="width:20px;">Rejected Date</th> 
         ...
         <div class="modal-dialog modal-sm">
               ...
               <div class="modal-content">
                 ...
                 <div class="modal-footer">
                   {!! Form::open(['method' => 'POST', 'action' => ['FinanceController@delete_rejected',0],'id'=>'delete_form']) !!}
      ...
      <div id="restoreModal" class="delete-modal modal fade" role="dialog">
         <div class="modal-dialog modal-sm">
               ...
               <div class="modal-content">
                 ...
                 <div class="modal-footer">
                   {!! Form::open(['method' => 'POST', 'action' => ['FinanceController@rejected_restore',0],'id'=>'restore_form']) !!}
                       <button type="reset" class="btn btn-gray translate-y-3" data-dismiss="modal">No</button>
                       <button type="submit" class="btn btn-danger">Yes</button>
                   {!! Form::close() !!}
                 </div>
      ...
      <div id="reason_model" class="delete-modal modal fade" role="dialog">
         <div class="modal-dialog modal-sm">
                           <!-- Modal content-->
               <div class="modal-content">
                 <div class="modal-body text-center">
                   <h4 class="modal-heading">Rejected Reason</h4>
                   <p id="rejected_reasone"></p>
                 </div>
                  <div class="modal-footer">
...
<script type="text/javascript">
  $.ajaxSetup({
   ...
   $(document).on('click','.delete_report',function(){
      ...
      $('#delete_form').attr('action','{{route("delete_rejected")}}/'+$(this).attr('id'))
      $('#deleteModal').modal('show');
   })
   $(document).on('click','.restore_report',function(){
      event.preventDefault();
      $('#restore_form').attr('action','{{route("rejected_restore")}}/'+$(this).attr('id'))
      $('#restoreModal').modal('show');
   }) 
    $(document).on('click','.show_reason',function(){
      event.preventDefault();
      $.get("{{route('get_rejected_reason')}}/"+$(this).attr('id'),function($result){
        if($result.status==true){
          $('#rejected_reasone').text($result.result.reject_reason)
        }
      })
...
$(document).on('click','#delete_reports',function(){
  ...
  if (checks.length === 0) {
   ...
   if(confirm("Are you sure that you want to Delete Reports?")){
      $.post('{{route("rejected_delete")}}',{report:checks},function(res){
         if(res.status){
